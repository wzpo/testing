<template>
  <div 
    class="audio-speaker-dropdown" 
    :style="{width: width}"
    @mousedown="click"
  >
    <div v-if="headerVisible" class="header" @click="close" >
      <span class="ico"><wl-icon name="speaker-bold" size="12" /></span>
      <span class="lb">Speaker</span>
      <wl-icon name="arrow-up-filled" size="8" />
    </div>

    <div 
      v-for="(itm, idx) in $store.state.speakers"
      :key="idx"
      class="item" 
      :class="{active: selectedSpeaker === itm}" 
      @click="selectSpeaker(itm)" 
    >
      <span v-if="iconVisible" class="ico"></span>
      <span class="lb">{{itm}}</span>
      <wl-icon v-if="selectedSpeaker === itm" name="check-bold" size="12" :color="theme === 'dark' ? 'blue-40':'blue-60'" />
    </div>
  </div>
</template>
