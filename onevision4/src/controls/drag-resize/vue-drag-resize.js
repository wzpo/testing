...
export default {
    ...
    mounted: function () {
        ...
        // call stop dragging from out side comp
        this.$eventBus.$on('force-stop-dragging', () => {
            this.bodyDrag = false;
        });
    },
    ...
    beforeDestroy: function () {
        ...
        this.$eventBus.$off('force-stop-dragging');
    },
    ...
    methods: {
        ...
        deselect(evt) {
            ...
            if (!this.hitTest(this.$refs.framewindow, evt.pageX, evt.pageY)) {
                this.active = false
            }
        },
        ...
        mousedown: function (ev) {
            ...
            if (ev.button && ev.button !== 0 || target.nodeName === 'INPUT') {
                return
            }
            ...
            ev.stopPropagation();
            //ev.preventDefault(); // why use this?
        ...
        calcResizeLimitation() {
            ...
            let limits = {
                minLeft: left + (width - maxw), //parentLim,
                maxLeft: left + (width - minw),
                minRight: right + (width - maxw), //parentLim,
                maxRight: right + (width - minw),
                minTop: top + (height - maxh), //parentLim,
                maxTop: top + (height - minh),
                minBottom: bottom + (height - maxh), //parentLim,
       ...
       /**
         ...
         * right 是窗口外边距离parent右边的距离
         * bottom 是窗口外边距离parent底部的距离
         */
        setRect(rect) {
            this.left = rect.left
    ...
    computed: {
        style() {
            ...
            if (this.active) {
                newstyles = merge(styles, this.activeStyleObject)
            } else {
                newstyles = merge(styles, this.deactiveStyleObject)
            }
        ...
        vdrStick() {
            return (stick) => {
                ...
                if (this.borderIsHandle) {
                    if (stick === 'tm' || stick === 'bm') {
                        stickStyle.width = `100%`;
                        stickStyle.left = `0`;
                    }
                    if (stick === 'mr' || stick === 'ml') {
                        stickStyle.height = `100%`;
                        stickStyle.top = `0`;
                    }
                }

                if (this.aspectRatio) {
                    if (stick === 'tm' || stick === 'bm' || stick === 'mr' || stick === 'ml') {
                        stickStyle.display = `none`;
                    }
                }
